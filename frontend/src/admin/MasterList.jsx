...
const MasterList = () => {
  ...
  const handleViewBills = async (customer) => {
    ...
    try {
      ...
      // Refresh customer list to show updated stats
      fetchCustomers(page);
    } catch {
    ...
    <Box sx={{ p: 2, background: "#f4f6f9", minHeight: "100vh" }}>
          ...
          <>
            <Table size="small">
              <TableHead sx={{ bgcolor: "#0B2A4A" }}>
                <TableRow>
                  {["#", "Customer", "Phone", "Total Bills", "Total Spent", "Pending Amount", "Last Purchase", "Actions"].map((h) => (
      ...
      <Dialog open={historyOpen} onClose={() => setHistoryOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            <Typography fontWeight={800} color="#0B2A4A">
        ...
        {!loadingBills && (
          <Box sx={{ display: "flex", gap: 2, px: 3, py: 2, flexWrap: "wrap" }}>
            <Paper sx={{ flex: 1, p: 2, borderRadius: 2, textAlign: "center", bgcolor: "#e3f2fd" }}>
              <Typography fontSize={22} fontWeight={800} color="#1565c0">
                {stats.totalBills || 0}
              </Typography>
              ...
              <TableHead sx={{ bgcolor: "#0B2A4A" }}>
                <TableRow>
                  {["Invoice #", "Date", "Amount", "Paid", "Pending", "Status", "Action"].map((h) => (
                    <TableCell key={h} sx={{ color: "#fff", fontWeight: 700, fontSize: 12 }}>
                      {h}
                    </TableCell>
                  ))}
                </TableRow>
              ...
              <TableBody>
                {bills.map((bill) => (
                  <TableRow key={bill._id} hover>
                    ...
                    <TableCell sx={{ color: bill.pendingAmount > 0 ? "#c62828" : "#2e7d32", fontWeight: 600 }}>
                      ₹{bill.pendingAmount}
                    </TableCell>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button
            ...
            onClick={() => {
              setHistoryOpen(false);
              navigate("/admin/billing", { state: { customer: selectedCustomer } });
            }}
            sx={{ bgcolor: "#C4161C", "&:hover": { bgcolor: "#a31217" } }}
          >
            Create New Bill
